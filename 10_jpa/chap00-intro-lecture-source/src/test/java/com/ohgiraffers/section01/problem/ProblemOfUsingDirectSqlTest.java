...
public class ProblemOfUsingDirectSqlTest {
    ...
    @AfterEach
    void closseConnection() throws SQLException {
        con.rollback();
        con.close();
    }

    /* 설명.
     *  JDBC API를 이용해 직접 SQL을 다룰 떄 발생할 수 있는 문제점들
    ...
    @DisplayName("직접 SQL을 작성하여 메뉴를 조회할 때ㅔ 발생하는 문제 확인")
    @Test
    void testDirectSelectSql() throws SQLException {
        ...
        while (rs.next()) { // 변수 변경 시 여러 군데를 변경해야 한다.
            Menu menu = new Menu();
    ...
    @DisplayName("집접 SQL을 작성하여 신규 메뉴를 추가할 때 발생하는 문제 확인")
    @Test
     ...
     *  또한 객체를 사용할 때 SQL에 의존하면 객체에 ㅏㄱㅄ이 무엇이 들어있는지 확인하기 위해 SQL을 매번 살펴야 한다.
     * */
    /* 설명. 2-1. 조회 항목 변경 시 */
    ...
    @Test
    void testAssocationObject() throws SQLException {
        ...
        while (rs.next()) {
            ...
            ));  // 서로 다른 테이블은 다른 객체이므로 매핑에 번거로움이 있다.
     ...
     *     private String orderableStatus;
     *   }
     *   Join을 할수는 있지만 카테고리 번호를 볼 수가 없음
     *
     *
     *   CQRS를 하는 이유: JPA가 조회에 약하다.
     *                   마이바티스는 조회에 강함
    * */
    ...
    void testEquals() throws SQLException {
        ...
        Assertions.assertNotEquals(menu1, menu2);   // 동일 행이지만 자바에서는 다른 객체
    }
