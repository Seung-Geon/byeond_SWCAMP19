...
public class BidrectionTest {
    ...
    public void 양방향_연관관계_매핑_조회_테스트() {
        ...
        /* 설명. 1. Menu를 기준으로 */
        /* 설명. 연관관계의 주인(자식엔티티, ManyToOne, FetchType.EAGER가 디폴드)은 left join문으로 조회한다. */
        ...
        /* 설명.
         *  양방향 관계는 toString()메소드에서 연관관계에 있는 엔티티의 toString을 호출하는 것을 신경써야 한다.
         *  1. 순환참조로 인한 stackOverflow를 발생시키므로 최소 한쪽 테ㅐ이블에서는 주석 처리 필요
         *  2. 아무리 FetchTYpe.LAZY 더라도 샌ㅅ갸ㅜㅎ의 여자관계에 있는 toString()을 호출하며
         *     추가 select 문을 발생시킨다.(N+1 문제)
         *  결롱은, 왠만하면 연관관계를 가진 엔티티는 toString에서 연관관계와 관련된 필드는 생략하는 편이 좋다.
        * */
    }
