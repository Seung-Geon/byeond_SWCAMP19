...
public class ManyToOneAssociationTest {
     ...
     *  이를 통해 객체를 이용해 데이터베이스의 테이블 간의 관걔를 활용해 한번에 객체로 조회할 수 있다.
     *
     * 설명.
     *  1. 다중성에 의한 분류
     *     : 연관관계가 있는 객체 관계에서 실제로 연관을 가지는 객체의 수에 따라 분류한다.
     *      - N:1(ManyToOne) 연관관계
     *      - 1:N(OneToMany) 연관관계 -> 안씀
     *      - 1:1(OneToOne) 연관관계
     *      - M:N(ManyToMany) 연관관계(교차 엔티티에 해당하는 중간 테이블을 자동으로 만듦) -> 안씀
     *  2. 방향에 따른 분류
     *     : 테이블의 연관관계는 외래 키를 이용하여 양방향 연관 관계의 특징을 가진다.
     *       반면, 객체는 참조에 의한 연관관계로 단방향이다.
     *       객체 간의 연간 관계를 양방향으로 만들고 싶은 경우 반대 쪽에서도 필드를 추가해서 참조를 보관하면 된다.
    ...
    * */
    ...
    public void 다대일_연관관계_객체_그래프_탐색을_이용한_조회_테스트() {
        ...
        /* 설명. 만약에 메뉴만 보고 싶다면? (N+1 문제를 해결하기 위한 방법)
         *      : FetchType.EAGER가 아닌 FetchType.LAZY로 변경해야 함(toString에서 접근하지 않게)
         *      조인도 하고 싶다면?
        ...
        *  */
        ...
        // FetchType을 LAZY로 해도 Join 해서 받아오기 가능 - 여기서는 inner 조인
        String jpql = "select m from menu_and_category m join fetch m.category where m.menuCode = " + menuCode;
        ...
        System.out.println("카테고리의 이름 확인: " + menu.getCategory().getCategoryName());
    }
