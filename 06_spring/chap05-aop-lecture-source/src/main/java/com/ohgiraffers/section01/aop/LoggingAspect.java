...
public class LoggingAspect {
    ...
    /* 설명. 1. Before Advice */  // target method전에 실행하는 Advice
//    @Before("execution(* com.ohgiraffers.section01.aop.*Service.*(..))")
    @Before("LoggingAspect.loggingAspect()")   // 원래는 위에 처럼 해야 하지만 loggingAspect을 만들어서 간단하게 작성
    public void logBefore(JoinPoint joinPoint) {    // 여기서의 joinPoint는 런타임 시점에 advice가 적용될 대상
    ...
    /* 설명. 5. Around Advice */      // 앞뒤로 붙이기
    /* 설명.
     *  이 메소드는 타겟 메소드를 완전히 장악하기 때문에 앞서 살펴 본 어드바이스 모두
     *  Around Advice 하나로 대체가 가능하다.
     *  타겟 메소드를 언제 실행할지, 아니면 아예 실행을 안할지 여부도 제어한다.(feat. 필터와 유사)
     *  하지만 타겟 메소드를 실해앟는 proceed() 메소드를 잊는 경우가 자주 발생하기 때문에 사용 시
     *  주의해야 하며 가능한 최소한의 요건ㅇ르 충족하면서도 가장 약한 어드바이스를 쓰는 것이 바람직하다.
    * */
