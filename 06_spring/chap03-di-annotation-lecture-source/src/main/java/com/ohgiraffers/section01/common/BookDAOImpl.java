...
public class BookDAOImpl implements BookDAO {
    ...
    public BookDAOImpl() {
        ...
        books.put(3, new BookDTO(3, 334455, "가나다라마바사",
                "이지", "이지출판",  new java.util.Date()));
    }

    @Override
    public List<BookDTO> findAllBook() {
        /* 설명. Map을 List 형태로 변환(feat. Collection 타입을 List 형태로 바꾸는 생성자 활용) */
        return new ArrayList<>(books.values());
    }

    @Override
    public BookDTO findBookBySequenceOf(int sequence) {
        return books.getOrDefault(sequence, null);
    }
}
