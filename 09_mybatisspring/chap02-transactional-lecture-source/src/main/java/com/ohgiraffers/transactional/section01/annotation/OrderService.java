...
public class OrderService {
    ...
    @Transactional(propagation = REQUIRED, isolation = Isolation.SERIALIZABLE, rollbackFor = Exception.class)   // rollbackFor을 이용해 롤백 조건을 걸 수 있음
...
//        }
        List<Integer> menuCodes = orderInfo
                .getOrderMenus()
                .stream()
...
//        menus.forEach(System.out::println);
        ...
        /* 설명. 4-2. Order로 테이블에 insert */
        orderMapper.insertOrder(order); // selectKey를 활용해서 insert 이후 select을 통해 한번의 DB와의 I/O를 발생시킴
    ...
    /* 설명. 사용자가 주문한 각 메뉴의 수량에 따른 총 주문금액 꼐산용 메소드 */
