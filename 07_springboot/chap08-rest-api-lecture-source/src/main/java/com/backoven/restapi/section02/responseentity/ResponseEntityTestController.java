package com.backoven.restapi.section02.responseentity;
...
public class ResponseEntityTestController {
    ...
    * */
    ...
    public ResponseEntityTestController() {
        ...
        this.users.add(new UserDTO(1, "user01", "pass01", "홍길동", new java.util.Date()));
        this.users.add(new UserDTO(2, "user02", "pass02", "유관순", new java.util.Date()));
        this.users.add(new UserDTO(3, "user03", "pass03", "이순신", new java.util.Date()));
    }
    ...
    public ResponseEntity<ResponseMessage> findUserByNo(@PathVariable int userNo) {
        ...
        return ResponseEntity.ok()
                .headers(headers)
                .body(new ResponseMessage(200, userNo + "번 회원 조회 성공!", responseMap));
    }

    /* 설명. 응답할 데이터가 따로 없다면 ResponseEntity의 제네릭에 ?을 활용할 수 있다.
            (feat. ResponseEntity 생성 시 build()로 마무리) */
    @PostMapping("/user")
    public ResponseEntity<?> registUser(@RequestBody UserDTO newMember) {
        System.out.println("numMember: " + newMember);
