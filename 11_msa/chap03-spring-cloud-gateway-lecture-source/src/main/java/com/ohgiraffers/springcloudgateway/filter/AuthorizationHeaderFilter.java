...
public class AuthorizationHeaderFilter
    extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter.Config> {    // 톰켓 필터 X
    ...
    public static class Config {    // 부모의 제네릭 타입을 자식이 가지는 형태
    ...
    public GatewayFilter apply(Config config) {
        ...
        return (exchange, chain) -> {
            ...
            /* 설명. 토큰을 들고 왔따면 추가 검증 */
            HttpHeaders headers = request.getHeaders(); // springframework 패키지로 import 할것
    ...
    private boolean isJwtValid(String jwt) {
        ...
        /* 설명. 기본적으로 우리 서버에서 만들었고, 만료기간이 지나지 않았으며, 토큰 안에 'sub'라는 등록된 클레임이 있는지 확인 */
        try {
            subject = Jwts.parser()
